...
| R-2 | **Audit log gaps in BFF** | `LoggingModule` (pino) | Medium | BFF operations not logged to immutable store. **Partially mitigated:** `BffAuditLog` model + `AuditTrailService` now persists auth events (wallet login, zkLogin, workspace switch) to PostgreSQL. Pino still logs to stdout for general request/response. Future: extend to GDPR executor and admin operations. |
...
| D-4 | **Airdrop gas bomb** | `airdrop.move` | Medium | `batch_airdrop()` iterates over `recipients.length()`. **Mitigated:** `MAX_BATCH_SIZE = 500` enforced on-chain via `EBatchTooLarge` (1302). BFF should also enforce at API layer. |
...
| E-6 | **BFF testLogin in production** | `AuthService.testLogin()` | Critical | Bypasses signature verification entirely. **Mitigated:** Three-layer defense — AppModule conditional import (`NODE_ENV === 'test'`), TestAuthController constructor guard, and AuthService.testLogin() ForbiddenException guard. |
...
| ~~RR-3~~ | ~~**Quest badge revocation not implemented**~~ | ~~Medium~~ | ~~`quest_badge.move`~~ | **MITIGATED.** Added `revoke_badge()` (admin soft-revoke via shared registry + dedup entry removal for re-mint) and `burn_badge()` (owner self-destruct). `is_revoked()` accessor for consumers. 5 new tests. |
| ~~RR-4~~ | ~~**Seal policy permissive for workspace/role rules**~~ | ~~High~~ | ~~`policy.move`~~ | **MITIGATED.** `seal_approve()` now takes `&Workspace` and enforces on-chain membership via `workspace::is_member()`. `RULE_ROLE_BASED` additionally checks `acl::level(role) >= min_role_level`. Cross-workspace attacks prevented by `policy.workspace_id == workspace::id()` assertion. 7 tests (4 red-team attack vectors + 3 happy-path). |
...
| ~~RR-6~~ | ~~**No on-chain batch size limit for airdrops**~~ | ~~Medium~~ | ~~`airdrop.move`~~ | **MITIGATED.** `MAX_BATCH_SIZE = 500` constant added. Both `batch_airdrop()` and `batch_airdrop_variable()` assert `recipient_count <= MAX_BATCH_SIZE` with `EBatchTooLarge` (1302) abort. |
...
| ~~RR-9~~ | ~~**BFF audit trail not persistent**~~ | ~~Medium~~ | ~~`LoggingModule`~~ | **MITIGATED.** New `BffAuditLog` model + `AuditTrailService` (@Global). Persists off-chain operations (auth login, workspace switch, GDPR) to PostgreSQL. Fire-and-forget logging in auth flows. Queryable via `AuditTrailService.query()`. |
| ~~RR-10~~ | ~~**`testLogin()` exposure risk**~~ | ~~Critical~~ | ~~`AuthService`~~ | **MITIGATED.** Three-layer defense: (1) AppModule only imports TestAuthModule when `NODE_ENV === 'test'`; (2) TestAuthController constructor throws if `NODE_ENV !== 'test'`; (3) `AuthService.testLogin()` throws `ForbiddenException` if `NODE_ENV !== 'test'`. Default test address fixed to valid hex. |
