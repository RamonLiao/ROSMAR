...
const STABLECOIN_TOKENS = new Set([
  'USDC', 'USDT', 'DAI', 'BUSD', 'AUSD', 'FUSD', 'BUCK', 'USDY',
]);

const MAJOR_TOKENS = new Set([
  'SUI', 'WETH', 'ETH', 'WBTC', 'BTC', 'SOL',
]);
...
const MOCK_POOLS: PoolApy[] = [
  ...
  { protocol: 'Cetus', poolName: 'SUI-WETH', tokenPair: 'SUI/WETH', apy: 18.5, tvl: 8_200_000, riskLevel: 'medium' },
  { protocol: 'Cetus', poolName: 'SUI-CETUS', tokenPair: 'SUI/CETUS', apy: 35.2, tvl: 4_100_000, riskLevel: 'high' },
  { protocol: 'Cetus', poolName: 'CETUS-USDC', tokenPair: 'CETUS/USDC', apy: 22.1, tvl: 3_500_000, riskLevel: 'high' },
  { protocol: 'Aftermath', poolName: 'SUI-afSUI', tokenPair: 'SUI/afSUI', apy: 7.6, tvl: 45_000_000, riskLevel: 'low' },
  { protocol: 'Scallop', poolName: 'USDC Lending', tokenPair: 'USDC', apy: 5.1, tvl: 62_000_000, riskLevel: 'low' },
  { protocol: 'Scallop', poolName: 'SUI Lending', tokenPair: 'SUI', apy: 3.8, tvl: 28_000_000, riskLevel: 'medium' },
...
export class YieldOptimizerService {
  ...
  private cache: { data: PoolApy[]; fetchedAt: number } | null = null;
  ...
  async getPoolApys(): Promise<PoolApy[]> {
    ...
    const pools = await this.fetchCetusPools();
    this.cache = { data: pools, fetchedAt: Date.now() };
    return pools;
  }
  ...
  private buildRationale(
    ...
    const suiAmount = Math.round(balanceSui * (allocationPct / 100) * 100) / 100;
    const parts: string[] = [];

    parts.push(`Allocate ~${allocationPct}% (~${suiAmount} SUI) to ${pool.protocol} ${pool.poolName}.`);
    parts.push(`Current APY: ${pool.apy}%.`);
    parts.push(`TVL: $${pool.tvl.toLocaleString('en-US')}.`);

    if (pool.riskLevel === 'low') {
      parts.push('Low risk — stablecoin or liquid-staking pair with minimal impermanent loss.');
    } else if (pool.riskLevel === 'medium') {
      parts.push('Medium risk — involves major tokens; moderate impermanent loss possible.');
    } else {
      parts.push('High risk — volatile pair with significant impermanent loss potential. Higher reward to compensate.');
    }
